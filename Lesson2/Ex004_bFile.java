...
public class Ex004_bFile {
    ...
    static void saveFile(String path) throws IOException {  // Сохранение файла
        if (data.size() == 0)
    ...
    static void delete(String word) {  // Удаление файла
        data.remove(word);
    }

    static void add(String word, String text) throws Exception {  // Добавление в файл
        if (data.get(word) != null)
            throw new Exception("Word already exist");
        data.put(word, text);
    }

    static String find(String word) {  // Поис в файле
    ...
    static int readInt(InputStream in) throws IOException {  // Чтение отдельных блоков
        byte out[] = new byte[4];
        int i = in.read(out);
        if (i != 4)
            throw new IOException("Error read file");
    ...
    static void writeInt(OutputStream out, int num) throws IOException { // Записываение отдельных блоков
